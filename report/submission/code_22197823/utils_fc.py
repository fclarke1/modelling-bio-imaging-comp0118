...
def model_multi_compartment(T2: NDArray, S0: NDArray, TE: NDArray, v: NDArray=np.array([[1]])) -> NDArray:
    ...
    assert np.isclose(np.prod(v.sum(axis=-1).flatten()), 1), f'every voxel compartment decomposition has to sum to 1 : {v}'
...
def objective_three_compartment(x, signal, TE_times, S0, T2_0, T2_1, WM_GM_factor):
    """objective used to minimize for three compartment, only free params are T_2, v_2
        ...
        try:
            ...
            for subject_id in tqdm(subject_ids, ascii=True):
                ...
                for roi_id in self.roi_id_dict: 
                    roi_data_subject[roi_id].append(mri_data_flatten_mono[is_roi[...,roi_id],:])
                    roi_seg_subject[roi_id].append(seg_data_flatten_mono[is_roi[...,roi_id],:])

            roi_data = []
            roi_seg = []
            for roi_id in self.roi_id_dict:
                roi_data.append(np.concatenate(roi_data_subject[roi_id], axis=0))   # roi_data_subject is an array of arrays so needs to be concatenated
